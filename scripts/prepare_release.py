...
def prepare_release(
    ...
    *,
    ...
    genre = overrides.get("genre") or _value(manifest, credits, "genre", "Electronic")
    language = overrides.get("language") or _value(manifest, credits, "language", "zho")
    release_date = overrides.get("release_date") or manifest.get("release_date") or ""
    copyright_line = overrides.get("copyright") or manifest.get("copyright") or f"© {date.today().year} {artist}"
...
def main() -> None:
    ...
    metadata = prepare_release(args.song_dir, output_dir=args.output_dir, allow_unapproved=args.allow_unapproved, overrides=overrides)
    print(json.dumps({"audio": metadata["audio"], "metadata": "release_metadata.json", "csv": "release_metadata.csv"}, ensure_ascii=False, indent=2))
